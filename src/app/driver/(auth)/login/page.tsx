...
import { isDriverRole, safeDriverRedirect } from '@/config/security'
import { getSessionUser } from '@/server/dal/session'
import { LoginForm } from './login-form'

export default async function DriverLoginPage({ searchParams }: { searchParams: Promise<{ callbackUrl?: string }> }) {
  const { callbackUrl } = await searchParams
  // Authoritative (database-backed) check, so a revoked session stays here.
  const user = await getSessionUser()
  if (user && isDriverRole(user.role)) redirect(safeDriverRedirect(callbackUrl))
