...
import { driverLogin } from '@/server/actions/auth'

export function LoginForm({ callbackUrl }: { callbackUrl?: string }) {
  const t = useTranslations('Driver')
  const locale = useLocale()
  const [state, action, pending] = useActionState(driverLogin, null)
      ...
      <form action={action} className="space-y-4 rounded-3xl border border-line bg-ink-2/90 p-7 shadow-2xl backdrop-blur">
        <div className="flex items-center justify-between">
          <h1 className="flex items-center gap-2 font-semibold">
            <Truck size={16} className="text-rose-500" /> {t('signInTitle')}
          </h1>
